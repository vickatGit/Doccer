...
const UserMessage: React.FC<Props> = ({ msg }) => {
  ...
  return (
    <div
      className={`flex justify-end mt-6  p-${isHighlighted ? 2 : 0} ${
        isHighlighted ? "bg-[#eae9fc]" : "bg-transparent"
      }`}
      id={msg?._id}
    >
      <div
        className={`rounded-tl-[0.6rem] rounded-tr-[0.6rem] rounded-bl-[0.6rem] bg-accent-purple relative py-1.5 px-5 max-w-[700px] min-w-[5rem]`}
      >
